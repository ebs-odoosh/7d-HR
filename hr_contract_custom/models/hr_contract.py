        ...
        if decision == 'confirm':
            ...
            if not self.related_trial_period.extended_from:
                self.env['trial.period'].create({
                    ...
                    'extended_from': self.related_trial_period.id
                })
                self.related_trial_period.related_contract.write(
                ...
                self.related_trial_period.related_contract.message_notify(
                    partner_ids=self.related_trial_period.related_employee.user_id.partner_id.ids,
    ...
    def set_extend(self):
        if self.response_id.state == 'done':
            if not self.extended_from:
                ...
                self.related_contract.message_notify(
                    partner_ids=self.related_employee.user_id.partner_id.ids,
                    body="Probation assessment done, The probation period will been extended till %s" % (
...
class Contract(models.Model):
    ...
    # Comment, because _onchange_manager_id in custom module appraisal, which is not migrated
    ...
    def _check_current_contract(self):
        ...
        for contract in self.filtered(lambda c: c.state not in ['draft', 'cancel'] or c.state == 'draft'):
            domain = [
                ('id', '!=', contract.id),
                ...
                ('state', 'in', ['open', 'close']),
                ('state', '=', 'draft')
                # ('kanban_state', '=', 'done')  # replaces incoming # Comment because field not exists
            ]
    ...
    def _get_total_wages(self):
        for rec in self:
            if rec.related_compensation:
                basic_salary = sum(
                    rec.related_compensation.filtered(lambda x: x.name.component_type == 'basic_pay').mapped('amount'))
                percentage = sum(rec.related_compensation.filtered(
                    lambda x: x.is_payroll and x.amount_type == 'percentage').mapped('amount'))/100.0
                rec.wage = sum(
                    rec.related_compensation.filtered(lambda x: x.is_payroll and x.amount_type == 'amount').mapped(
    ...
    def _get_compensations(self):
        line = [{'name': "Pay Component", 'amount': "Amount", 'description': "Description", 'bold': True,'percent_currency':''}]

        for comp in self.related_compensation.filtered(lambda x: x.is_payroll):
            line.append({'name': comp.name.name,
                         'amount': comp.amount,
                         'description': comp.component_description,
                         'percent_currency': '%' if comp.amount_type == 'percentage' else self.currency_id.symbol,
                         'bold': False})

        line.append({'name': "", 'amount': self.wage, 'description': "Total Monthly Salary (AED)", 'percent_currency': self.currency_id.symbol,
                     'bold': True})
        return line

    def _get_benefits(self):
        line = []
        for comp in self.related_compensation.filtered(lambda x: not x.is_payroll):
    ...
    def action_contract_signed(self):
        # todo task #34
        # Comment due to model defined in custom_org_chart
