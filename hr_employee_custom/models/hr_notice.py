...
class HrNotices(models.Model):
    ...
    def _on_employee_change(self):
        employee = self.env['hr.employee'].browse(self._context.get('default_related_employee', []))
        if employee:
            self.related_manager = employee.parent_id
            self.related_department = employee.department_id
            self.related_job_title = employee.job_title
