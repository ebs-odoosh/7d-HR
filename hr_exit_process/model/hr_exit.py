...
class hr_exit_line(models.Model):
    ...
    def checklist_approved(self):
        for line in self.checklist_unique_ids:
            ...
            if line.mandatory and not line.comments:
                raise Warning(_(
                    'Cant complete the Exit checklist line, please fill the mandatory lines'))
        self.state = 'approved'
        # add by bhavesh Jadav 29/01/2021
        activities = self.env['mail.activity'].sudo().search(
            [('res_model', '=', 'hr.exit.line'), ('res_id', '=', self.id),
        ...
        for checklist in checklists:
            ...
            for checkline in checklines:
                checklist.checklist_unique_ids = [
                    (0, 0,
                     {'name': checkline.name, 'is_comments': checkline.is_comments, 'mandatory': checkline.mandatory})]
        return res
